...
force_len_threshold = 10
end_force_threshold = 4
output = 0
...
while i < len(on_key):
  if on_key[i] == 1 :
      current_dur.append(df['Y'][i])
      current_dfi.append(df['Y.1'][i])
      current_dpi.append(df['Y.2'][i])
      current_vur.append(diff['V_Y'][i])
      current_vfi.append(diff['V_Y.1'][i])
      current_vpi.append(diff['V_Y.2'][i])
